...
export default function Driver() {
  ...
  async function savePremi() {
    try {
      ...
      // Ambil hanya baris yang mempunyai rute
      const rows = premiRows
        .filter((r) => r.nama_rute?.trim())
        .map((r) => ({
          id: r.id,
          driver: selectedDriver.trim(),
          nama_rute: r.nama_rute.trim(),
          premi: Number(r.premi || 0),
        }));

      /*
      * 1. UPDATE baris lama
      *    Kalau row mempunyai id, berarti row tersebut sudah ada
      *    di database sehingga cukup UPDATE.
      */
      ...
      for (const row of existingRows) {
        ...
        if (error) {
          console.error("Gagal update premi rute:", error);
          alert("Gagal update premi rute: " + error.message);
          return;
        }
      }
      ...
      /*
      * 2. INSERT baris baru
      *    Row tanpa id berarti baru ditambahkan melalui
      *    tombol "Tambah Rute".
      */
      const newRows = rows
      ...
      if (newRows.length > 0) {
        ...
        if (error) {
          console.error("Gagal insert premi rute:", error);
          alert("Gagal menambah premi rute: " + error.message);
          return;
        }
      }

      /*
      * 3. HAPUS row yang memang sudah dihapus dari popup
      *
      * Ambil semua row database milik driver ini.
      */
      const { data: databaseRows, error: fetchError } = await supabase
        .from("driver_premi_rute")
        .select("id")
        .eq("driver", selectedDriver.trim());

      if (fetchError) {
        console.error("Gagal cek premi lama:", fetchError);
        alert("Gagal mengecek premi lama: " + fetchError.message);
        return;
      }
      ...
      const idsToDelete = (databaseRows || [])
        .map((row) => row.id)
        .filter((id) => !currentIds.has(id));

      /*
      * Hanya delete row yang benar-benar dihapus dari popup.
      */
      if (idsToDelete.length > 0) {
        ...
        if (error) {
          console.error("Gagal hapus premi rute:", error);
          alert("Gagal menghapus premi rute: " + error.message);
          return;
        }
      }
      ...
      console.error("Error savePremi:", error);

      const message =
        error instanceof Error
          ? error.message
          : "Terjadi kesalahan saat menyimpan premi.";
